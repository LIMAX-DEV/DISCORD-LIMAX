...
color_webhook = 0x8A2BE2  # Changed to purple
username_webhook = name_tool

color = colorama.Fore
purple = color.MAGENTA  # Purple color
white = color.WHITE
green = color.GREEN
reset = color.RESET
yellow = color.YELLOW
...
INFO_ADD = f'{white}[{purple}+{white}]{purple}'
...
def MainColor(text):
    start_color = (138, 43, 226)  # Dark purple
    end_color = (216, 191, 216)   # Light purple
...
def ChoiceMultiTokenDisord():
    def CheckToken(token_number, token):
        ...
        if response.status_code == 200:
            ...
            print(f" {BEFORE}{token_number}{AFTER} -> {purple}Status: {white}Valid{purple} | User: {white}{username_discord}{purple} | Token: {white}{token_sensur}")
        else:
            print(f" {BEFORE}{token_number}{AFTER} -> {purple}Status: {white}Invalid{purple} | {purple}Token: {white}{token}")
    ...
    with open(file_token_discord, 'r') as file_token:
        ...
        if token_discord_number == 0:
            print(f"{BEFORE + current_time_hour() + AFTER} {INFO} No Token Discord in file: {white}{file_token_discord_relative}{purple} Please add tokens to the file.")
            Continue()
            Reset()
        else:
            print(f"{BEFORE + current_time_hour() + AFTER} {INFO} {white}{token_discord_number}{purple} Token Discord found ({white}{file_token_discord_relative}{purple})")
    ...
    with open(file_token_discord, 'r') as file_token:
        print()
        print(f"{BEFORE + current_time_hour() + AFTER} {INFO} Token Discord ({white}{file_token_discord}{purple}):\n")
        for line in file_token:
            if not line.strip() or line.isspace():
                continue
...
def Choice1TokenDiscord():
    ...
    with open(file_token_discord, 'r') as file_token:
        print(f"{BEFORE + current_time_hour() + AFTER} {INFO} Token Discord ({white}{file_token_discord_relative}{purple}):\n")
        for line in file_token:
            if not line.strip() or line.isspace():
                continue
